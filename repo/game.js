...
class GameRepo {
    ...
    static getGameSeriesPlatform(search, cb) {
        db.all("SELECT g.refSeries AS series, g.orgPlatform AS platform FROM game g WHERE g.title = ?", search, cb)
    }

    static async getGameSeriesPlatformPromise(search) {
        return new Promise((resolve, reject) => {
            this.getGameSeriesPlatform(search, (err, res) => {
                if (err) {
                    reject(err)
                } else {
                    resolve(res)
    ...
    static getGameDevelop(search, cb) {
        db.all("SELECT g.refSeries AS series, g.orgPlatform AS platform FROM game g WHERE g.title = ?", search, cb)
    }
